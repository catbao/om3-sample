...
export async function batchLoadDataForRangeLevel1MinMaxMiss123(losedRange: Array<Array<number>>, manager: any, tagName?: string){
    let difVals: Array<{ l: number, i: number, dif?: Array<number> }>
    if(store.state.controlParams.currentLineType==='Single'){
        difVals= await batchLoadMinMaxMissWithWs123(losedRange, manager.dataName, manager.maxLevel, tagName) as Array<{ l: number, i: number }>;
    }else{
        difVals= await batchLoadMinMaxMissWithPostForMultiLineType(losedRange, manager.dataName, "level_load_data_min_max_miss", manager.maxLevel, tagName)
    }
