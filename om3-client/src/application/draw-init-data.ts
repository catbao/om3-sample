...
export function drawViewChangeLineChartInit(lineChartObj: { width: number, height: number, x: number, y: number, minv: number, maxv: number }) {
    ...
    let ctx: any = null;
    let canvas :any= null
    const draw = function (nonUniformColObjs: Array<{ind:number,v:number}>,name:string) {
        return
        if (Math.log2(lineChartObj.width) === Math.floor(Math.log2(lineChartObj.width))) {
            xScale.domain([0, nonUniformColObjs.length]).range([0, lineChartObj.width]);
        } else {
            xScale.domain([0, nonUniformColObjs.length]).range([0, lineChartObj.width]);
        }
        ...
        for (let i = 0; i < nonUniformColObjs.length; i++) {
            ...
            if(i==0){
                ctx.moveTo(xScale(i),yScale(nonUniformColObjs[i].v))
            }else{
                ctx.lineTo(xScale(i),yScale(nonUniformColObjs[i].v))
            }
        }
        //console.log(nonUniformColObjs)
        ctx.stroke();
