...
const loadViewChangeQueryWSMinMaxMissDataInitData: ActionHandler<GlobalState, GlobalState> = (context: ActionContext<GlobalState, GlobalState>, payload: { startTime: number, endTime: number, width: number, height: number }) => {
    ...
    const combinedUrl = `/line_chart/init_wavelet_bench_min_max_miss?width=${2 ** (Math.ceil(Math.log2(10486)))}&table_name=${context.state.controlParams.currentTable}&mode=${context.state.controlParams.currentMode}`;
    ...
    data.then(tempRes => {
        if (tempRes['code'] === 200) {
            ...
            dataManager.getDataMinMaxMiss(currentLevel + 1, 0, 2 ** (currentLevel + 1) - 1).then(() => {
                ...
                dataManager.viewChangeInteractionFinal1(14, payload.width, [0, lineInfo['max_len'] - 1], yScale, drawer).then(res => {
                    drawer(res)
